...
* Transitional data stores are optimized for writes. These write-friendly schemas need significant transformation logic to be read-friendly, and the transformation logic needs to scale to large eng teams
* Classic ETL + pre-computation is not feasible due to freshness requirements
* Reporting query is complex, often joining 50+ tables from different data stores
