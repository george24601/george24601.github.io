...
In one dimension, a banded shift-invariant matrix is a Toeplitz matrix or a filter. Each shift has a diagonal of 1 's A = x_1Lv + x_0C + x_{-1}R
...
When the input v is an image, the convolution with x becomes two-dimensional. E^2 independent weights. v represents (N+2)^2 pixels, and output have only N^2 pixels

The 2D convolution is a linear combination of 9 shifts.
...
Each shift is a matrix with a diagonal full of 1 's
...
The dot products between a smooth function and a moving filter window will be smooth.  But when an edge in the image lines up with a diagonal wall, we see a spike.
 ...
 filters have many purposes, including smoothing, gradient detection, and edge detection.
